...
<head>
    ...
    <title>Atur Ulang Kata Sandi - Sistem Informasi Klinik</title>
...
<body class="bg-white min-h-screen flex flex-col">
    ...
    <header class="h-16 bg-white flex items-center px-6">
        <a href="{{ route('password.forgot.verify') }}" class="text-[#005b66] hover:text-[#004a54] transition-colors">
    ...
    <div class="flex-1 medical-pattern flex items-center justify-center p-6 md:p-12">
        <div class="w-full max-w-6xl grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
            ...
            <!-- Sisi Kiri: Form Atur Ulang Kata Sandi -->
            <div class="bg-white md:bg-transparent p-8 md:p-0 rounded-2xl md:rounded-none shadow-xl md:shadow-none max-w-md mx-auto w-full">
                <h2 class="text-3xl font-bold text-[#005b66] mb-2">Kata Sandi Baru</h2>
                <p class="text-gray-600 mb-8">Buat kata sandi baru untuk akun Anda. Harap gunakan minimal 6 karakter.</p>
                ...
                <form action="{{ route('password.forgot.reset.save') }}" method="POST" class="space-y-6">
                    @csrf
                    <!-- Input Password Baru -->
                    <div>
                        <label for="password" class="block text-xl font-medium text-[#005b66] mb-2">Kata Sandi Baru</label>
                        <input type="password" name="password" id="password" required placeholder="Masukkan kata sandi baru..."
                    ...
                    <div>
                        <label for="password_confirmation" class="block text-xl font-medium text-[#005b66] mb-2">Konfirmasi Kata Sandi</label>
