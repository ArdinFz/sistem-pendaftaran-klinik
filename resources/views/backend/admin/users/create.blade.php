...
@section('title', 'Tambah Akun - Klinik Tadika Mesra')
...
<div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6 space-y-6">
    ...
    <form action="{{ route('admin.users.store') }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 md:grid-cols-3 gap-8">
        @csrf
        ...
        <div class="flex flex-col items-center space-y-4">
            ...
            <label for="foto" class="w-full aspect-square border-2 border-dashed border-gray-300 hover:border-[#005b66] rounded-xl flex flex-col items-center justify-center text-gray-500 bg-gray-50 hover:bg-gray-100 cursor-pointer overflow-hidden relative transition-colors">
                <!-- Preview Image (Tersembunyi Awalnya) -->
                <img id="preview" src="#" alt="Preview Foto" class="hidden w-full h-full object-cover">
                
                <!-- Placeholder Teks & Icon -->
                <div id="placeholder" class="flex flex-col items-center justify-center space-y-2 p-4 text-center">
        ...
        <div class="md:col-span-2 space-y-5">
            ...
            <div>
                ...
                <input type="text" name="nik" id="nik" value="{{ old('nik') }}" required placeholder="Masukkan NIK"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:border-[#005b66] bg-white">
            </div>
            ...
            <div>
                ...
                <input type="password" name="password" id="password" required placeholder="Masukkan Password (Min. 6 Karakter)"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:border-[#005b66] bg-white">
            </div>
            ...
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name') }}" required placeholder="Masukkan Nama"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:border-[#005b66] bg-white">
            </div>
