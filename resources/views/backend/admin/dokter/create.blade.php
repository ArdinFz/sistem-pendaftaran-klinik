...
@section('title', 'Tambah Dokter - Klinik Tadika Mesra')

@section('content')
<div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6 space-y-6">
    
    <div class="border-b border-gray-200 pb-4">
        <h1 class="text-xl font-bold text-gray-800">Tambah Dokter</h1>
    </div>
    ...
    <form action="{{ route('admin.dokter.store') }}" method="POST" class="max-w-xl space-y-5">
        @csrf
        
        <!-- Nama Dokter -->
        <div>
            <label for="name" class="block text-sm font-bold text-gray-700 mb-1.5">Nama Dokter</label>
            <input type="text" name="name" id="name" value="{{ old('name') }}" required placeholder="Masukkan Nama Dokter"
        ...
        <div>
            ...
            <input type="text" name="spesialis" id="spesialis" value="{{ old('spesialis') }}" required placeholder="Masukkan Keahlian (Contoh: Poli Umum)"
                class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:border-[#005b66] bg-white">
        </div>
