...
@section('title', 'Master Data Dokter - Klinik Tadika Mesra')
...
<div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6 space-y-6">
    ...
    <!-- Header & Pencarian -->
    <form action="{{ route('admin.dokter.index') }}" method="GET" class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 border-b border-gray-100 pb-4">
        <div>
        ...
        <div class="flex items-center space-x-2">
            ...
            <input type="text" name="search" id="search" value="{{ $search }}" placeholder="Cari nama/keahlian/nomor hp..."
                class="px-3 py-1.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:border-[#005b66] bg-white">
            @if($search)
                <a href="{{ route('admin.dokter.index') }}" class="text-xs text-red-500 hover:underline">Clear</a>
            @endif
        </div>
    </form>
    ...
    <!-- Judul Halaman -->
    <div>
        <h1 class="text-lg font-bold text-gray-800">Dokter</h1>
    </div>

    <!-- Tabel Grid Dokter (Boxy Format) -->
        ...
        <div class="overflow-x-auto">
            <div class="space-y-2 min-w-[850px]">
                ...
                <div class="grid grid-cols-12 gap-2 text-center text-gray-700 font-bold text-sm">
                    ...
                    <div class="bg-white border border-gray-200 rounded-lg p-3 text-left col-span-3">Nama Dokter</div>
                    <div class="bg-white border border-gray-200 rounded-lg p-3 text-left col-span-3">Keahlian</div>
                    <div class="bg-white border border-gray-200 rounded-lg p-3 col-span-2">Nomor Hp</div>
                ...
                @forelse ($dokters as $dokter)
                    <div class="grid grid-cols-12 gap-2 text-center items-center text-gray-600 text-sm">
                        ...
                        <!-- Nama Dokter -->
                        <div class="bg-white border border-gray-200 rounded-lg p-3 text-left font-medium col-span-3">{{ $dokter['name'] }}</div>
                        
                        <!-- Keahlian -->
                        <div class="bg-white border border-gray-200 rounded-lg p-3 text-left col-span-3">{{ $dokter['spesialis'] }}</div>
                        
                        <!-- Nomor Hp -->
                        <div class="bg-white border border-gray-200 rounded-lg p-3 col-span-2">{{ $dokter['no_hp'] }}</div>
                        ...
                        <div class="bg-white border border-gray-200 rounded-lg p-2.5 flex items-center justify-center space-x-2 col-span-3">
                            ...
                            <form action="{{ route('admin.dokter.destroy', $dokter['id']) }}" method="POST" 
                                onsubmit="return confirm('Apakah Anda yakin ingin menghapus data dokter ini?');" class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit" 
