...
@section('title', 'Jadwal Dokter - Klinik Tadika Mesra')
...
<div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6 space-y-6">
    ...
    <!-- Header & Pencarian -->
    <form action="{{ route('admin.jadwal-dokter.index') }}" method="GET" class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 border-b border-gray-100 pb-4">
        <div>
            <!-- Tombol Tambah (Hijau dengan icon tambah.png) -->
            <a href="{{ route('admin.jadwal-dokter.create') }}" 
        ...
        <div class="flex items-center space-x-2">
            ...
            <input type="text" name="search" id="search" value="{{ $search }}" placeholder="Cari dokter/poli/hari..."
                class="px-3 py-1.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:border-[#005b66] bg-white">
            @if($search)
                <a href="{{ route('admin.jadwal-dokter.index') }}" class="text-xs text-red-500 hover:underline">Clear</a>
            @endif
        </div>
    </form>
    ...
    <!-- Judul Halaman -->
    <div>
        <h1 class="text-lg font-bold text-gray-800">Jadwal Dokter</h1>
    </div>
