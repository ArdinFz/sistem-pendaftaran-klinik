...
@section('title', 'Edit Jadwal Dokter - Klinik Tadika Mesra')

@section('content')
<div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6 space-y-6">
    
    <div class="border-b border-gray-200 pb-4">
        <h1 class="text-xl font-bold text-gray-800">Edit Jadwal Dokter</h1>
    </div>
    ...
    <form action="{{ route('admin.jadwal-dokter.update', $jadwal['id']) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="space-y-5">
                ...
                <div>
                    ...
                    <select name="dokter" id="dokter" required
                        ...
                        @foreach($dokters as $d)
                            <option value="{{ $d }}" {{ old('dokter', $jadwal['dokter']) == $d ? 'selected' : '' }}>{{ $d }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <!-- Poliklinik -->
                <div>
                    <label for="poliklinik" class="block text-sm font-bold text-gray-700 mb-1.5">Poliklinik</label>
                    <select name="poliklinik" id="poliklinik" required
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:border-[#005b66] bg-white">
                        <option value="">-- Pilih Poliklinik --</option>
                        @foreach($polikliniks as $p)
                            <option value="{{ $p }}" {{ old('poliklinik', $jadwal['poliklinik']) == $p ? 'selected' : '' }}>{{ $p }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="kuota" id="kuota" value="{{ old('kuota', $jadwal['kuota']) }}" required min="1" placeholder="Kuota pendaftaran"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:border-[#005b66] bg-white">
                </div>
            </div>
            ...
            <div class="space-y-5">
                <!-- Tanggal -->
                <div>
                    <label for="tanggal" class="block text-sm font-bold text-gray-700 mb-1.5">Tanggal</label>
                    <input type="date" name="tanggal" id="tanggal" value="{{ old('tanggal', $jadwal->tanggal ? $jadwal->tanggal->format('Y-m-d') : '') }}" required
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:border-[#005b66] bg-white">
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="jam_mulai" id="jam_mulai" value="{{ old('jam_mulai', $jadwal['jam_mulai']) }}" required placeholder="Contoh: 08:00"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:border-[#005b66] bg-white">
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="jam_selesai" id="jam_selesai" value="{{ old('jam_selesai', $jadwal['jam_selesai']) }}" required placeholder="Contoh: 10:00"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:border-[#005b66] bg-white">
                </div>
            </div>
