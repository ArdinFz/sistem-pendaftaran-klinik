...
@section('title', 'Tambah Poliklinik - Klinik Tadika Mesra')

@section('content')
<div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6 space-y-6">
    
    <div class="border-b border-gray-200 pb-4">
        <h1 class="text-xl font-bold text-gray-800">Tambah Poliklinik</h1>
    </div>
    ...
    <form action="{{ route('admin.poliklinik.store') }}" method="POST" class="max-w-xl space-y-5">
        @csrf
        
        <!-- Nama Poliklinik -->
        <div>
            <label for="nama_poli" class="block text-sm font-bold text-gray-700 mb-1.5">Nama Poliklinik</label>
            <input type="text" name="nama_poli" id="nama_poli" value="{{ old('nama_poli') }}" required placeholder="Masukkan Nama Poliklinik"
        ...
        <div>
            ...
            <textarea name="deskripsi" id="deskripsi" rows="4" required placeholder="Layanan..."
                class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:border-[#005b66] bg-white">{{ old('deskripsi') }}</textarea>
        </div>
